...
- (id) initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        // 设置选中时的字体颜色
        [self setTitleColor:[UIColor colorWithHex:0x299dff] forState:UIControlStateSelected];
        
        // 添加一个提醒数字按钮
        UIButton *badgeButton = [[UIButton alloc]init];
        // 设置提醒数字背影图片，只需要设置一次
        [self.badgeButton setBackgroundImage:[UIImage imageNamed:@"main_badge"] forState:UIControlStateNormal];
        // 设置fram
        CGFloat badgeX = 0;
        CGFloat badgeY = 0;
        CGFloat badgeW = 38;
