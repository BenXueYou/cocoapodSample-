...
- (void)commonInit{
    ...
    _xLineLabelHeight = 20.f;
    _xLineTitleFontSize = 10.f;
    _xLineValueFontSize = 10.f;
}

- (instancetype)initWithFrame:(CGRect)frame withtextx:(NSString *)textX withtexty:(NSString *)textY{
...
- (void)setXLineTitleLabel{
    for (NSInteger i = 0; i < self.xLineTitleArray.count; i++) {
        CGFloat xPos = self.axisLine.xLineStartXPos + _XLineItemGapLength + (_XLineItemWidth + _XLineItemGapLength) * i;
        CGFloat yPos = self.axisLine.xLineStartYPos + 5;
        CGFloat width = _XLineItemWidth;
...
//            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(self.axisLine.xLineEndXPos - 30, self.frame.origin.y + 15, 30, 15)];
//            label.font = [UIFont systemFontOfSize:_xLineTitleFontSize];
//            label.text = self.textX;
//            NSLog(@"坐标轴self.text == %@",self.textX);
//            [self addSubview:label];
...
- (void)setXLineValueLabel{
    for (NSInteger i = 0; i < self.xLineValueArray.count; i++) {
        ...
        CGFloat yPos = self.axisLine.xLineStartYPos + _xLineLabelHeight;
...
- (void)setYLineValueLabel{
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(self.frame.origin.x + 30, self.frame.origin.y + 15, 30, 15)];
    
    label.font = [UIFont systemFontOfSize:_xLineTitleFontSize];
//    self.textY = @"元";
    label.text = self.textY;
    NSLog(@"坐标轴self.textY == %@",self.textY);
