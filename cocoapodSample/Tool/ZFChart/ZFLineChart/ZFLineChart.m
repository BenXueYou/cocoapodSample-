...
-(instancetype)initWithFrame:(CGRect)frame withTextX:(NSString *)testx withTextT:(NSString *)texty{
...
- (void)drawGenericChart{
    self.genericChart = [[ZFGenericChart alloc] initWithFrame:self.bounds withtextx:self.textX withtexty:self.textY];
    self.genericChart.XLineItemWidth = 10;
    self.genericChart.XLineItemGapLength = 10;
    
    self.genericChart.textX = self.textX;
    self.genericChart.textY = self.textY;
    NSLog(@"画图self.text == %@",self.textX);
    NSLog(@"画图self.textY == %@",self.textY);
...
- (void)drawLineAndCirque{
    ...
    for (NSInteger i = 0; i < self.cirqueArray.count - 1; i++) {
        ...
        ZFLine * line = [[ZFLine alloc] initWithStartPoint:CGPointMake(cirque.center.x, cirque.center.y) endPoint:CGPointMake(end_XPos, end_YPos) lineColor:_lineColor];
...
- (void)showLabelOnChart{
    ...
    for (NSInteger i = 0; i < self.cirqueArray.count; i++) {
        ...
        if (_valueOnChartPosition == kLineChartValuePositionDefalut) {
            ...
            label_center = end_YPos <= 0 ? CGPointMake(cirque.center.x, cirque.center.y + 10) : CGPointMake(cirque.center.x, cirque.center.y - 10);
            
        //_valueOnChartPosition为圆环上方
        }else if (_valueOnChartPosition == kLineChartValuePositionOnTop){
            label_center = CGPointMake(cirque.center.x, cirque.center.y - 10);
