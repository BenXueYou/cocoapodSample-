...
- (void)addUI{
    for (NSInteger i = 0; i < self.valueArray.count; i++) {
        //装载容器
        UIView * background = [[UIView alloc] initWithFrame:CGRectMake(0, _pieCenter.y + _radius * 1.5 + 25 * i, self.frame.size.width, 20)];
        ...
        name.font = [UIFont boldSystemFontOfSize:14.f];
        ...
        value.font = [UIFont boldSystemFontOfSize:14.f];
        [background addSubview:value];
        
        //百分比
        UILabel * percent = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(value.frame) + gap, 0, width, 15)];
        percent.text = [self getPercent:i];
        percent.font = [UIFont boldSystemFontOfSize:13.f];
    ...
    self.contentSize = CGSizeMake(self.frame.size.width, CGRectGetMaxY(lastLabel.frame) + 10);
}
