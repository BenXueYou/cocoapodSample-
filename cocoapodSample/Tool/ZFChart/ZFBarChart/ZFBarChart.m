...
- (void)drawGenericChart{
    self.genericChart = [[ZFGenericChart alloc] initWithFrame:self.bounds withtextx:@"星期" withtexty:@"元"];
    self.genericChart.XLineItemWidth = 20;
    self.genericChart.XLineItemGapLength = 15;
...
- (void)drawBar{
    ...
    for (NSInteger i = 0; i < self.xLineValueArray.count; i++) {
        CGFloat xPos = self.genericChart.axisStartXPos + 15 + (20 + 15) * i;
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(xPos, yPos - 30, 50, height)];
        label.text = [NSString stringWithFormat:@"%@",self.xLineValueArray[i]];
...
- (void)showLabelOnChart{
    for (NSInteger i = 0; i < self.barArray.count; i++) {
        ...
        CGRect rect = [self.xLineValueArray[i] stringWidthRectWithSize:CGSizeMake(XLineItemWidth + XLineItemGapLength * 0.05, 100) fontOfSize:_valueOnChartFontSize isBold:NO];
